...
for startYear, endYear in [[1964, 2019], [1999, 2006], [2006, 2011], [2011, 2019]]:
    df_new = pd.read_csv('C:/share_VM/work/Project_PPI/forex_signals_revise02.csv', index_col=0)
    ...
    fName = str(startYear) + 's_corr_high_02.png'
    makeFig(plt, title, xlabel, ylabel, fName)

    plt.figure(figsize=(20, 8))
    correlations[-25:].plot.barh()
    fName = str(startYear) + 's_corr_low_02.png'
    makeFig(plt, title, xlabel, ylabel, fName)

    plt.figure(figsize=(20, 8))
    correlations[1:].plot.bar()
    plt.xticks([])
    fName = str(startYear) + 's_histogram_correlations_02.png'
    ...
    fName = str(startYear) + 's_histogram_predictions_02.png'
    makeFig(plt, title, xlabel, ylabel, fName)
